		...
		>
			<view class="item column" v-for="(item, index) in list" :key="index" @click="navTo('detail?id='+item._id)">
				...
				<view class="btn-group row" @click.stop.prevent="stopPrevent">
					<view v-if="item.status===4 || item.status===10 || item.status===11" class="btn center round" @click="deleteOrder(index)">
						<text>删除订单</text>
					</view>
...
<script>
	...
	export default {
		...
		mixins: [MescrollMixin, OrderMixin, customServiceMixin],
		data() {
			return {
				curPage: 'orderList',
				navCounts: [], //订单数量
